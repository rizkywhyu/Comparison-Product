...
class Batterypesaing extends CI_Controller
{
    ...
    {
        $batterypesaing = $this->Battery_pesaing_model->get_all();

        $data = array(
            'batterypesaing_data' => $batterypesaing
        );

        $this->template->load('template','batterypes/batterypesaing_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Battery_pesaing_model->get_by_id($id);
        if ($row) {
            $data = array(
            'model' => $row->model,
            'gambar' => $row->gambar,
            ...
            $this->template->load('template','batterypes/batterypesaing_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('batterypesaing'));
        }
    }
    ...
    public function create() 
    {
        ...
        $data = array(
            'button' => 'Create',
            'action' => site_url('batterypesaing/insert'),
            // 'dd_tempat' => $this->Battery_pesaing_model->dd_tempat(),
            ...
            'display' => set_value('display'),
            'feature' => set_value('feature'),
        );
        $this->template->load('template','batterypes/batterypesaing_form', $data);
    }
    ...
    function createaksi()
    {
        ...
        $this->Battery_pesaing_model->create_data($data);
        redirect('batterypesaing');
    }
        else {
                $this->create();
     ...
     */
        ...
        {
            if ($this->Battery_pesaing_model->valid_id($model) == TRUE)
            {
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('batterypesaing/updateaksi'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Battery_pesaing_model->update($this->input->post('model', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Battery_pesaing_model->delete($id);
    ...
    public function insert(){
         ...
         if($_FILES['filefoto']['name'])
        {
            ...
            if ($this->upload->do_upload('filefoto'))
            {
                ...
                $this->Battery_pesaing_model->create_data($data); //akses model untuk menyimpan ke database
                //pesan yang muncul jika berhasil diupload pada session flashdata
                $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-success\" id=\"alert\">Upload gambar berhasil !!</div></div>");
                redirect('batterypesaing'); //jika berhasil maka akan ditampilkan view vupload
            }else{
               ...
               echo '<script type="text/javascript"> alert("Tipe File Tidak Sama/Ukuran File Terlalu Besar") 
                window.location = "'.base_url().'batterypesaing'.'" </script>';
     ...
     public function updateaksi(){
       ...
       if($_FILES['filefoto']['name'])
       {
           if ($this->upload->do_upload('filefoto'))
           {
               ...
               $this->Battery_pesaing_model->get_update($data,$where); //akses model untuk menyimpan ke database
           ...
           redirect('batterypesaing'); /* jika berhasil maka akan kembali ke home upload */
       }
   }
