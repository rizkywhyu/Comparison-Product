...
class Diesel extends CI_Controller
{
    ...
    {
        $diesel = $this->Diesel_model->get_all();

        $data = array(
            'diesel_data' => $diesel
        );

        $this->template->load('template','diesel/diesel_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Diesel_model->get_by_id($id);
        if ($row) {
            $data = array(
    		'model' => $row->model,
            'gambar' => $row->gambar,
            ...
            $this->template->load('template','diesel/diesel_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('diesel'));
        }
    }
    ...
    public function create() 
    {
        ...
        $data = array(
            'button' => 'Create',
            'action' => site_url('diesel/insert'),
            // 'dd_tempat' => $this->Diesel_model->dd_tempat(),
        ...
        $this->template->load('template','diesel/diesel_form', $data);
    }
    ...
    function createaksi()
    {
        ...
        $this->Diesel_model->create_data($data);
        redirect('diesel');
    }
        else {
                $this->create();
     ...
     */
        ...
        {
            if ($this->Diesel_model->valid_id($model) == TRUE)
            {
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('diesel/updateaksi'),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Diesel_model->update($this->input->post('model', TRUE), $data);
     ...
     public function insert(){
         ...
         if($_FILES['filefoto']['name'])
        {
            ...
            if ($this->upload->do_upload('filefoto'))
            {
                ...
                $this->Diesel_model->create_data($data); //akses model untuk menyimpan ke database
                //pesan yang muncul jika berhasil diupload pada session flashdata
                $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-success\" id=\"alert\">Upload gambar berhasil !!</div></div>");
                redirect('diesel'); //jika berhasil maka akan ditampilkan view vupload
            }else{
               ...
               echo '<script type="text/javascript"> alert("Tipe File Tidak Sama/Ukuran File Terlalu Besar") 
                window.location = "'.base_url().'diesel'.'" </script>';
     ...
     public function updateaksi(){
       ...
       if($_FILES['filefoto']['name'])
       {
           if ($this->upload->do_upload('filefoto'))
           {
               ...
               $this->Diesel_model->get_update($data,$where); //akses model untuk menyimpan ke database
           ...
           redirect('diesel'); /* jika berhasil maka akan kembali ke home upload */
       }
   }
